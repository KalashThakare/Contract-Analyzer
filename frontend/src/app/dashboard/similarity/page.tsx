...
export default function SimilarityAnalysis() {
  ...
  const sortedClauses = [...result.clauses].sort((a,b) => a.confidence - b.confidence).slice(0, 10);
    ...
    <div className="space-y-6">
      <AnimatedContainer animation="fade-in" duration={0.2} className="border-b border-border pb-4">
        <h1 className="text-xl font-semibold tracking-tight text-foreground flex items-center gap-2">
           ...
           Baseline Policy Deviance
        </h1>
        <p className="text-xs text-muted-foreground mt-1 max-w-2xl">
          Sentence-BERT dot-product embeddings comparing ingestion nodes against standardized repository definitions. 
        </p>
      </AnimatedContainer>

      <AnimatedContainer animation="slide-up" staggerChildren={0.05} className="space-y-4">
        {sortedClauses.map((clause, i) => {
           const similarityPct = Math.round(clause.confidence * 100);
             ...
             <div key={i} className="glass-panel p-4 rounded-md shadow-sm flex flex-col md:flex-row gap-0 overflow-hidden relative group">
               ...
               <div className="flex-1 space-y-2 p-3 bg-muted/10 border-r border-border border-dashed">
                 <div className="flex items-center gap-2">
                   <h3 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest bg-muted px-1.5 py-0.5 rounded border border-border">Extracted Document</h3>
                 </div>
               ...
               <div className="flex flex-col items-center justify-center min-w-[100px] px-3 bg-background z-10 shrink-0">
                  <div className="text-center w-full">
                    ...
                    <span className="text-[9px] uppercase font-bold tracking-widest text-muted-foreground">Cosine Sim</span>
                  </div>
               ...
               <div className="flex-1 space-y-2 p-3 bg-muted/30 border-l border-border border-dashed opacity-80">
                 <div className="flex items-center gap-2">
                   <h3 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest bg-background border border-border px-1.5 py-0.5 rounded">Corporate Template</h3>
                 </div>
                 <div className="text-xs leading-relaxed text-muted-foreground min-h-[60px] italic">
                    ...
                    Confidential validation policy string omitted in UI layer. Represents baseline text optimized for zero-liability assertions.
                 </div>
               </div>
