    ...
    <div className="space-y-6">
      ...
      <AnimatedContainer animation="fade-in" duration={0.2} className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-4">
         <div>
            <h1 className="text-xl font-semibold tracking-tight text-foreground flex items-center gap-2">
              <Scale className="w-5 h-5 text-muted-foreground" /> Overview Telemetry
            </h1>
            <p className="text-xs text-muted-foreground mt-1 font-mono">{document.filename || uploadInfo?.filename || "Extracted_Document_Buffer"}</p>
         </div>
         <div className="flex items-center gap-3">
            <span className="px-2 py-1 bg-muted rounded border border-border text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
               {document.pages || uploadInfo?.pages || '?'} Pages Iterated
            </span>
            <span className="px-2 py-1 bg-primary/10 rounded border border-primary/20 text-[10px] font-bold uppercase tracking-widest text-primary">
               {document.total_clauses} Nodes Extracted
            </span>
         </div>
      </AnimatedContainer>
      ...
      <AnimatedContainer animation="slide-up" delay={0.1} staggerChildren={0.05} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="glass-panel p-4 flex flex-col gap-2 relative overflow-hidden group">
          <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest flex items-center gap-1.5"><Activity className="w-3.5 h-3.5" /> Aggregate Liability</h3>
        ...
        <div className="glass-panel p-4 flex flex-col gap-2 relative overflow-hidden group">
          <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest flex items-center gap-1.5"><AlertTriangle className="w-3.5 h-3.5 text-red-500" /> Critical Vectors</h3>
          <div className="flex items-end gap-2 mt-2">
            <span className="text-3xl font-bold tracking-tighter text-red-500 leading-none">{summary.high_risk_count}</span>
            <span className="text-xs text-muted-foreground font-mono mb-1">nodes</span>
          </div>
          <p className="text-[10px] text-muted-foreground mt-2 uppercase tracking-wide">Requires immediate counsel</p>
        </div>

        <div className="glass-panel p-4 flex flex-col gap-2 relative overflow-hidden group">
          <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest flex items-center gap-1.5"><ShieldAlert className="w-3.5 h-3.5 text-amber-500" /> Caution Vectors</h3>
          <div className="flex items-end gap-2 mt-2">
        ...
        <div className="glass-panel p-4 flex flex-col gap-2 relative overflow-hidden group">
          <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-widest flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" /> Validated Vectors</h3>
          <div className="flex items-end gap-2 mt-2">
      ...
      <AnimatedContainer animation="slide-up" delay={0.2} staggerChildren={0.05} className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="glass-panel p-4 flex flex-col">
          <h3 className="section-heading mb-0"><FileText className="w-4 h-4 inline-block mr-1.5" /> Liability Distribution</h3>
        ...
        <div className="glass-panel p-4 flex flex-col">
          <h3 className="section-heading mb-0"><Activity className="w-4 h-4 inline-block mr-1.5" /> Dominant Legal Taxonomies</h3>
