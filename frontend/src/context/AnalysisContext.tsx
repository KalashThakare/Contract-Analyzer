...
export function AnalysisProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (storedResult) {
      try { setResultState(JSON.parse(storedResult)); } catch (e) {}
    }
    
    if (storedUploadInfo) {
      try { setUploadInfoState(JSON.parse(storedUploadInfo)); } catch (e) {}
    }

    if (storedHistory) {
      try { setHistoryState(JSON.parse(storedHistory)); } catch (e) {}
    }
  }, []);
    ...
    if (newResult) {
      ...
      setHistoryState((prev) => {
        const item: ContractHistoryItem = {
          ...
          filename: newResult.document.filename || uploadInfo?.filename || "Unknown Contract",
  ...
  const loadContractFromHistory = async (id: string) => {
    ...
    try {
      ...
      setUploadInfoState({ filename: histItem?.filename || pastContact.document.filename || "History Contract", pages: pastContact.document.pages || 0 });
  ...
  return (
    <AnalysisContext.Provider
      value={{ result, setResult, uploadInfo, setUploadInfo, history, loadContractFromHistory, clearHistory, isLoadingHistory }}
    >
      {children}
    </AnalysisContext.Provider>
  );
